...
function getDropdownMenuOffset(side: MenuPrimitive.Popup.State['side']) {
  switch (side) {
    case 'top':
      return { x: 0, y: 4 }
    case 'left':
    ...
    <MenuPrimitive.Portal keepMounted>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn(
            'z-50 max-h-(--available-height) w-(--anchor-width) min-w-48 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-2xl bg-popover p-1 text-popover-foreground shadow-2xl ring-1 ring-foreground/5 outline-none data-closed:overflow-hidden dark:ring-foreground/10',
            className,
          )}
          render={(renderProps, state) => {
            const offset = getDropdownMenuOffset(state.side)
            ...
            return (
              <motion.div
                ...
                animate={{
                  ...
                  x: state.open ? 0 : offset.x,
                  y: state.open ? 0 : offset.y,
                }}
                initial={false}
                transition={
                  reduceMotion
